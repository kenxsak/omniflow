...
export default function CampaignJobsPage() {
  ...
  const getProviderLabel = (job: CampaignJob) => {
    if (job.jobType === 'email' && job.emailData?.provider) {
      const emailProviderMap: Record<string, string> = {
        'brevo': 'Brevo',
        'sender': 'Sender.net',
        'smtp': 'SMTP'
      };
    ...
    if (job.jobType === 'sms' && job.smsData?.provider) {
      const smsProviderMap: Record<string, string> = {
        'msg91': 'MSG91',
        'fast2sms': 'Fast2SMS',
        'twilio': 'Twilio'
      };
    ...
    if (job.jobType === 'whatsapp' && job.whatsappData?.provider) {
      const whatsappProviderMap: Record<string, string> = {
        'authkey': 'WMart CPaaS',
        'aisensy': 'Aisensy',
        'gupshup': 'Gupshup',
        'wati': 'WATI',
                    ...
                    <table className="w-full text-sm table-fixed">
                      <thead>
                        <tr className="bg-stone-100/50 dark:bg-stone-800/50">
                          ...
                          <th className="w-[25%] text-left py-3 px-3 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase align-middle">Campaign Name</th>
                          ...
                          <th className="w-[20%] text-left py-3 px-3 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase align-middle">Progress</th>
                          ...
                          <th className="w-[13%] text-right py-3 px-3 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase align-middle">Details</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            <tr key={job.id} className="border-t border-stone-200 dark:border-stone-800 hover:bg-muted/30 transition-colors group">
                              ...
                              <td className="text-right py-3 px-3 align-middle">
                                {lastError && (
                                  <p className="text-[10px] text-red-600 dark:text-red-400 truncate max-w-[120px] ml-auto">
                                    {lastError}
                                  </p>
                                )}
                              </td>
